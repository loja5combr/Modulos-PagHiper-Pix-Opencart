...
class ControllerPaymentPagHiperPix extends Controller {
	...
	public function index() {
		...
		//campos custo
		$cpf = $this->config->get('paghiperpix_origem_cpf');
		$cnpj = $this->config->get('paghiperpix_origem_cnpj');
		$fiscal = '';
		...
		if(version_compare(VERSION, '2.1.9.9', '<=')){
            return $this->load->view('default/template/payment/paghiperpix.tpl', $data);
		}else{
            return $this->load->view('payment/paghiperpix.tpl', $data);	
		}
	}
	...
	public function ipn(){
        $this->load->model('checkout/order');
		//modo debug 
		if($this->config->get('paghiperpix_debug')){
			$this->salvar_log('IPN PagHiper Pix:');
			$this->salvar_log(print_r($_REQUEST,true));
		}
		//se retornar os dados nescessarios
        if(isset($_POST['transaction_id']) && isset($_POST['notification_id']) && isset($_POST['apiKey']) && $_POST['apiKey']==trim($this->config->get('paghiperpix_chave'))){
			//json consulta a transacao pix
			$json = array();
			$json['token'] = trim($this->config->get('paghiperpix_token'));
			$json['apiKey'] = trim($this->config->get('paghiperpix_chave'));
			...
			if(isset($retorno['status_request']['result']) && $retorno['status_request']['result']=='success'){
				...
				if($retorno['status_request']['status']=='paid'){
					//se pago
					if($pedidos['order_status_id']!=$this->config->get('paghiperpix_aprovado')){
						$this->model_checkout_order->addOrderHistory((int)$retorno['status_request']['order_id'],$this->config->get('paghiperpix_aprovado'),'',true);
					}
					...
					//se cancelado
					if($pedidos['order_status_id']!=$this->config->get('paghiperpix_cancelado')){
						$this->model_checkout_order->addOrderHistory((int)$retorno['status_request']['order_id'],$this->config->get('paghiperpix_cancelado'),'',true);
					}
					...
					//se devolvido
					if($pedidos['order_status_id']!=$this->config->get('paghiperpix_devolvido')){
						$this->model_checkout_order->addOrderHistory((int)$retorno['status_request']['order_id'],$this->config->get('paghiperpix_devolvido'),'',true);
					}
	...
	private function criar_pix_paghiper($pedido){
		...
		$json['days_due_date'] = (int)$this->config->get('paghiperpix_dias');
	...
	public function confirm() {
		...
		if($this->config->get('paghiperpix_debug') || $pix['status']!=201){
        ...
        if($pix['status']==201 && isset($pix['retorno']['pix_create_request']['result']) && $pix['retorno']['pix_create_request']['result']=='success'){
			...
			$this->model_checkout_order->addOrderHistory($pedido_id, $this->config->get('paghiperpix_iniciado'), $msg, true);
